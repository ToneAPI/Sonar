...
def valueschartweapon(playerid, server = "", mapid = "", mapname="" ,gamemodeid = "", gamemode=""):
    ...
    if(mapname != "All"):
        payload['map'] = mapid
    if(gamemode != "Any"):
        payload['gamemode'] = gamemodeid
        
    response = requests.get(os.environ.get("TONE_ENDPOINT") + '/v2/client/weapons', params=payload).json()
    ...
    other = 0
    for j in result:
        if(dict_weapon_with_kills[j] > total/34):
            try:
